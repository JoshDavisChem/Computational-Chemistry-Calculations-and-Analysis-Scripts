...
if [ ! -d LiFeO2_Li"$atom"_CalcFold ]; then
   mkdir LiFeO2_Li"$atom"_CalcFold
fi
# make the initial skeleton file for lithium to be removed
cat > LiFeO2_Li_"$atom"PreEdit.in << EOF
 &CONTROL
   title = "LiFeO2_Li$atom",
...
sed "$((59 + $atom))d" LiFeO2_Li_"$atom"PreEdit.in >  LiFeO2_Li_"$atom".in

#put it in the correct folder

mv ./LiFeO2_Li_"$atom".in ./LiFeO2_Li"$atom"_CalcFold/

# make a submission file for it

cd ./LiFeO2_Li"$atom"_CalcFold/

fold=$(pwd)

cat > LiFeO2_Li_"$atom".qsub << EOF
#!/bin/bash -login
#PBS -l walltime=24:00:00,nodes=4:ppn=4,mem=16gb
#PBS -N LiFeO2_Li_$atom
...
mpirun -np 16 pw.x -nk 4 -nd 16 <LiFeO2_Li_$atom.in>LiFeO2_Li_$atom.out

EOF

#submit it
qsub LiFeO2_Li_"$atom".qsub
cd ..

rm LiFeO2_Li_"$atom"PreEdit.in
